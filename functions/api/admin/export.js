...
async function exportClients(env) {
  ...
  const rows = [[
    ...
    "engagement_count", "total_invoiced_cents", "total_paid_cents",
  ]];

  for (const r of results) {
    rows.push([
      r.id, r.name, r.company, r.email, r.phone, r.notes, r.created_at,
...
async function exportEngagements(env) {
  const { results } = await env.DB.prepare(
    `SELECT e.id AS engagement_id, c.name AS client_name, c.company AS client_company, c.email AS client_email,
            e.title, e.stage, e.invoice_number, e.invoice_date, e.invoice_amount_cents,
  ...
  const rows = [[
    "engagement_id", "client_name", "client_company", "client_email", "title", "stage",
    "invoice_number", "invoice_date", "invoice_amount_cents", "invoice_status",
    "payment_method", "payment_link", "payment_reference", "contract_signed_at", "created_at",
  ]];

  for (const r of results) {
    rows.push([
      r.engagement_id, r.client_name, r.client_company, r.client_email, r.title, r.stage,
      r.invoice_number, r.invoice_date, r.invoice_amount_cents, r.invoice_status,
      r.payment_method, r.payment_link, r.payment_reference, r.contract_signed_at, r.created_at,
    ]);
  }
